...
export default function CreateRole({ open, onClose, onSuccess }: CreateRoleProps) {
  ...
  // Load all permissions once on mount
  useEffect(() => {
    const loadPermissions = async () => {
      setLoadingPermissions(true);
      setErrorMessage(null);
      try {
        // Fetch all permissions (non-paginated backend path)
        const response = await PermissionsService.getAllPermissions();
        setAllPermissions(response.results);
      } catch (error: unknown) {
        console.error("Error loading permissions:", error);
        setErrorMessage("No se pudieron cargar los permisos del sistema.");
      } finally {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const apiErrorMsg = err.response?.data?.message || err.message || "Error inesperado al crear el rol.";
    ...
    <Dialog open={open} onClose={saving ? undefined : onClose} maxWidth="md" fullWidth>
      ...
      <form onSubmit={(e) => void handleSubmit(e)}>
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          ...
          >
            {saving ? <CircularProgress size={20} color="inherit" /> : "Guardar Rol"}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
