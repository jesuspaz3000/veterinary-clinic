...
import { useGroomingSpecialties } from "../hooks/groomingSpecialtiesHooks";
import { GroomingSpecialtyResponse } from "../type/groomingSpecialtiesTypes";

interface ManageGroomingSpecialtiesDialogProps {
  open: boolean;
  onClose: () => void;
  onSpecialtyChange?: () => void;
}

export default function ManageGroomingSpecialtiesDialog({
  open,
  onClose,
  onSpecialtyChange,
}: ManageGroomingSpecialtiesDialogProps) {
  const { specialties, loading, fetchSpecialties } = useGroomingSpecialties();
  ...
  // Popover state for assigned grooming staff list
  ...
  const handleStartEdit = (spec: GroomingSpecialtyResponse) => {
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await GroomingSpecialtiesService.updateSpecialty(editingId, {
          name: name.trim(),
          description: description.trim() || null,
        });
      } else {
        await GroomingSpecialtiesService.createSpecialty({
          name: name.trim(),
          description: description.trim() || null,
        });
      }
      ...
      setErrorMessage(err.response?.data?.message || err.message || "Error al guardar la especialidad de peluquería.");
    } finally {
      setSaving(false);
    }
  };

  const handleDelete = async (spec: GroomingSpecialtyResponse) => {
    if (spec.assignedCount > 0) return;

    setSaving(true);
    setErrorMessage(null);
    try {
      await GroomingSpecialtiesService.deleteSpecialty(spec.id);
      await fetchSpecialties();
      if (onSpecialtyChange) onSpecialtyChange();
    } catch (error: unknown) {
      console.error(error);
  ...
  const handleOpenPopover = (event: React.MouseEvent<HTMLElement>, spec: GroomingSpecialtyResponse) => {
    setPopoverAnchor(event.currentTarget);
    setActiveStaffList(spec.assignedStaffNames || []);
  ...
  return (
    <Dialog open={open} onClose={saving ? undefined : onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ fontWeight: 700, pr: 5 }}>Catálogo de Especialidades de Peluquería</DialogTitle>
      <IconButton
        aria-label="Cerrar"
        onClick={onClose}
        disabled={saving}
        ...
        >
          ...
          <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, gap: 2, alignItems: "flex-start" }}>
            <TextField
              ...
              placeholder="Ej. Corte de Raza, Stripping, Baño Medicado"
              value={name}
              onChange={(e) => {
          ...
          {loading ? (
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              {specialties.map((spec) => {
                const inUse = spec.assignedCount > 0;
                return (
                  <Paper
                    key={spec.id}
                    variant="outlined"
                  ...
                  >
                    <Box sx={{ display: "flex", flexDirection: "column", flex: 1 }}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        ...
                        <Chip
                          label={`${spec.assignedCount} asignado(s)`}
                          size="small"
                    ...
                    <Box sx={{ display: "flex", gap: 0.5 }}>
                      ...
                      <Tooltip
                        title={
                          inUse
                            ? `No se puede eliminar porque está asignada a ${spec.assignedCount} personal(es) de grooming`
      ...
      {/* Popover displaying assigned grooming staff */}
      <Popover
      ...
      >
        <Box sx={{ p: 2, maxWidth: 300 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1, color: "primary.main" }}>
            Personal con {activeSpecialtyName}:
          </Typography>
