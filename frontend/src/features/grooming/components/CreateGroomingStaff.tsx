...
import { GroomingStaffCreateRequest } from "../type/groomingTypes";
...
interface CreateGroomingStaffProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function CreateGroomingStaff({
  open,
  onClose,
  onSuccess,
}: CreateGroomingStaffProps) {
  // Catalog state
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const dto: GroomingStaffCreateRequest = {
    ...
    try {
      await GroomingService.createGroomingStaff(dto);
      resetForm();
      onSuccess();
      onClose();
    } catch (error: unknown) {
      console.error("Error creating grooming staff:", error);
      const err = error as { response?: { data?: { message?: string } }; message?: string };
      const apiErrorMsg =
        err.response?.data?.message || err.message || "Error inesperado al crear personal de grooming.";
  ...
  return (
    <>
      <Dialog open={open} onClose={saving ? undefined : onClose} maxWidth="md" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, pr: 5 }}>Nuevo Personal de Grooming / Estética</DialogTitle>
        <IconButton
          aria-label="Cerrar"
          onClick={onClose}
          disabled={saving}
