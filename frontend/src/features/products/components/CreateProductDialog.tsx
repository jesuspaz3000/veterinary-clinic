...
export default function CreateProductDialog({
  ...
  const [targetSpecies, setTargetSpecies] = useState("Todas");
  const [description, setDescription] = useState("");
  const [requiresPrescription, setRequiresPrescription] = useState(false);
  const [allowsFractioning, setAllowsFractioning] = useState(false);

  const [image, setImage] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  const [zoomOpen, setZoomOpen] = useState(false);

  // Variants state (at least 1 empty variant with clean placeholder)
  ...
  useEffect(() => {
    ...
    const fetchCatalogs = async () => {
      try {
        ...
        console.error("Error loading catalogs for create product:", err);
      }
    };
    void fetchCatalogs();
    return () => {
  ...
  const resetForm = () => {
    ...
    setTargetSpecies("Todas");
    setDescription("");
    setRequiresPrescription(false);
    setAllowsFractioning(false);
    setImage(null);
    setImagePreview(null);
    setVariants([
      {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    for (let i = 0; i < variants.length; i++) {
      const v = variants[i];
      const variantName = v.name.trim() || (variants.length === 1 ? "Estándar / Unidad" : `Presentación #${i + 1}`);
      if (v.salePrice === undefined || v.salePrice === null || v.salePrice < 0) {
        setErrorMessage(`Ingresa un precio de venta válido para "${variantName}".`);
        return;
      }
    }
    ...
    const processedVariants = variants.map((v, i) => ({
      ...v,
      name: v.name.trim() || (variants.length === 1 ? "Estándar / Unidad" : `Presentación #${i + 1}`),
    }));
    ...
    try {
      ...
      const apiErrorMsg =
        err.response?.data?.message || err.message || "Error inesperado al crear el producto.";
  ...
  return (
    <>
      <Dialog open={open} onClose={saving ? undefined : onClose} maxWidth="lg" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, pr: 5 }}>Nuevo Producto de Inventario / Farmacia</DialogTitle>
        <IconButton
          aria-label="Cerrar"
          onClick={onClose}
          disabled={saving}
        ...
        <form noValidate onSubmit={(e) => void handleSubmit(e)}>
                  ...
                  {variants.map((variant, index) => (
                    ...
                    >
                      ...
                      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr 1fr" }, gap: 2, mb: 1.5 }}>
                        <TextField
                          label="Nombre de presentación / variante"
                          placeholder={index === 0 ? "Ej. Caja x 100 tab, Frasco 50ml, Bolsa 3kg" : `Ej. Presentación #${index + 1}`}
                          size="small"
                          value={variant.name}
          ...
          <DialogActions sx={{ px: 3, pb: 3 }}>
            ...
            >
              {saving ? <CircularProgress size={20} color="inherit" /> : "Guardar Producto"}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
