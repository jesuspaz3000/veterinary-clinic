...
import { AdministrativeStaffCreateRequest } from "../type/administrativeTypes";
...
interface CreateAdministrativeStaffProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function CreateAdministrativeStaff({
  open,
  onClose,
  onSuccess,
}: CreateAdministrativeStaffProps) {
  // Catalogs state
  ...
  const [selectedArea, setSelectedArea] = useState<AdministrativeAreaResponse | null>(null);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const dto: AdministrativeStaffCreateRequest = {
    ...
    try {
      await AdministrativeService.createAdministrativeStaff(dto);
      resetForm();
      onSuccess();
      onClose();
    } catch (error: unknown) {
      ...
      const apiErrorMsg =
        err.response?.data?.message || err.message || "Error inesperado al crear personal administrativo.";
  ...
  return (
    <>
      <Dialog open={open} onClose={saving ? undefined : onClose} maxWidth="md" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, pr: 5 }}>Nuevo Personal Administrativo</DialogTitle>
        <IconButton
          aria-label="Cerrar"
          onClick={onClose}
          disabled={saving}
        ...
        <form noValidate onSubmit={(e) => void handleSubmit(e)}>
          ...
          <DialogActions sx={{ px: 3, pb: 3 }}>
            ...
            >
              {saving ? <CircularProgress size={20} color="inherit" /> : "Guardar Personal"}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
