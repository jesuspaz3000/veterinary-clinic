...
import { VeterinarianCreateRequest } from "../type/veterinariansTypes";
import ManageSpecialtiesDialog from "@/features/specialties/components/ManageSpecialtiesDialog";

interface CreateVeterinarianProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
export default function CreateVeterinarian({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!username.trim() || !email.trim() || !password.trim() || !licenseNumber.trim()) {
    ...
    const dto: VeterinarianCreateRequest = {
    ...
    try {
      await VeterinariansService.createVeterinarian(dto);
      resetForm();
      onSuccess();
      onClose();
    } catch (error: unknown) {
      ...
      const apiErrorMsg =
        err.response?.data?.message || err.message || "Error inesperado al crear el veterinario.";
  ...
  return (
    <>
      <Dialog open={open} onClose={saving ? undefined : onClose} maxWidth="md" fullWidth>
        <DialogTitle sx={{ fontWeight: 700 }}>Nuevo Veterinario</DialogTitle>
        <form noValidate onSubmit={(e) => void handleSubmit(e)}>
          ...
          <DialogActions sx={{ px: 3, pb: 3 }}>
            ...
            >
              {saving ? <CircularProgress size={20} color="inherit" /> : "Guardar Veterinario"}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
